...
%The intent of the game was to discover websites that would fool a hate-speech classifier. Through trial and error, a utility function was derived to value high confidence mistakes (UUs) from diverse URLs. Then users ``played'' the game by submitting URLs to earn a monetary reward tied to the utility function. 
...
%  The feature space is not restricted to match that of the classifier to accommodate cases when the original features are unavailable.  
...
%We use the log scaling to avoid over-incentivizing the search for incredibly rare UUs, as we know there is a limited budget for oracle queries.
...
%A summary for each is given below.    
%
%\begin{itemize}
...
%This affirms that the facility locations utility model is rewarding the selection of overconfident UUs. We also see that the utility values from facility locations search for Pang04 and Pang05 data were slightly lower.
